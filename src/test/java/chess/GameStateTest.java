...
public class GameStateTest {
    ...
    public void testInitialGame() {
        ...
        PieceColor current = state.getCurrentPieceColor();
        assertEquals("The initial player should be White", PieceColor.White, current);

        // Spot check a few pieces
        Piece whiteRook = state.getPieceAt("a1");
        assertTrue("A rook should be at a1", whiteRook instanceof Rook);
        assertEquals("The rook at a1 should be owned by White", PieceColor.White, whiteRook.getColor());


        Piece blackQueen = state.getPieceAt("d8");
        assertTrue("A queen should be at d8", blackQueen instanceof Queen);
        assertEquals("The queen at d8 should be owned by Black", PieceColor.Black, blackQueen.getColor());
    }
}
